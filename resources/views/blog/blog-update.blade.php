...
<div class="container mt-5">
    ...
    <form method="post" action="{{ route('blogs.update', $blog->id) }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" required>{{ $blog->description }}</textarea>
            @error('description')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="tags" name="tags" value="{{ $blog->tags->pluck('name')->implode(',') }}">
        </div>
            ...
            <div id="link-container">
                @foreach ($blog->links as $link)
                    <div class="input-group mt-2">
                        <input type="text" class="form-control" name="links[{{ $link->id }}][title]" value="{{ $link->title }}" placeholder="Link Title">
                        <input type="text" class="form-control" name="links[{{ $link->id }}][url]" value="{{ $link->url }}" placeholder="Link URL">
...
<script>
    $(document).ready(function() {
        var linkCount = {{ count($blog->links) }};
